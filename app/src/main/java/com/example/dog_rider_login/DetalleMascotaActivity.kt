...
class DetalleMascotaActivity : AppCompatActivity() {
        ...
        try {
            ...
            if (!foto.isNullOrEmpty()) {
                if (foto.startsWith("avatar_")) {
                    ...
                    dogImage?.let { Glide.with(this).load(url).centerCrop().into(it) }
                }
            } else {
            ...
            Log.e("DETALLE_PET", "Error: ${e.message}")
        }
    }
        ...
        RetrofitClient.instance.iniciarPaseo(request).enqueue(object : Callback<AuthResponse> {
            override fun onResponse(call: Call<AuthResponse>, response: Response<AuthResponse>) {
                if (response.isSuccessful && response.body()?.success == true) {
                    Toast.makeText(this@DetalleMascotaActivity, getString(R.string.msg_paseo_iniciado_exito, nombre), Toast.LENGTH_LONG).show()
                    finish()
                }
            }
            override fun onFailure(call: Call<AuthResponse>, t: Throwable) {
                Toast.makeText(this@DetalleMascotaActivity, getString(R.string.error_conexion_servidor), Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun finalizarPaseoReal(id: Int, nombre: String) {
        ...
        RetrofitClient.instance.finalizarPaseo(request).enqueue(object : Callback<AuthResponse> {
            override fun onResponse(call: Call<AuthResponse>, response: Response<AuthResponse>) {
                if (response.isSuccessful && response.body()?.success == true) {
                    Toast.makeText(this@DetalleMascotaActivity, getString(R.string.msg_paseo_finalizado_exito, nombre), Toast.LENGTH_LONG).show()
                    finish()
                }
            }
            override fun onFailure(call: Call<AuthResponse>, t: Throwable) {
